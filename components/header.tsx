...
const Header: React.FC = () => {
	...
	return (
		<header className='sticky top-0 left-0 right-0 z-50 bg-stone-100/50 backdrop-blur-sm'>
				...
				<div className='flex items-center justify-between h-16'>
					<div className='flex items-center'>
						<Link
							...
							<Bitcoin className='mr-2' /> Edu Housing
						</Link>
					</div>
							...
							{navItems.map((nav) => {
								return (
									<Link
										key={nav.name}
										href={nav.url}
										className='text-stone-900 hover:text-stone-700 font-semibold px-3 py-2 rounded-md text-sm transition-colors duration-300'>
										{nav.label}
									</Link>
								);
							})}
					...
					<div className='flex items-center'>
						...
						<div className='-mr-2 flex md:hidden'>
							<button
								type='button'
								className='inline-flex items-center justify-center p-2 rounded-md text-stone-500 hover:text-stone-400 hover:bg-stone-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white'
